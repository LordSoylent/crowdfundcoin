...
// Copyright (c) 2019 The CrowdFundingCoin developers
// Distributed under the MIT software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#if defined(HAVE_CONFIG_H)
#include "config/CrowdFundingCoin-config.h"
#endif
...
void PrepareShutdown()
{
    ...
    RenameThread("CrowdFundingCoin-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "CrowdFundingCoin.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "CrowdFundingCoind.pid"));
#endif
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 47755, 46755));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in CFUND/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in CFUND/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-rescan", _("Rescan the block chain for missing wallet transactions") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-salvagewallet", _("Attempt to recover private keys from a corrupt cfund.dat") + " " + _("on startup"));
    ...
    strUsage += HelpMessageOpt("-wallet=<file>", _("Specify wallet file (within data directory)") + " " + strprintf(_("(default: %s)"), "cfund.dat"));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, mempool, net, CrowdFundingCoin, (obfuscation, swifttx, masternode, mnpayments, mnbudget)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in CFUND/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all CrowdFundingCoin specific functionality (Masternodes, Obfuscation, SwiftTX, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s)"), "128.127.106.235:47755"));
    ...
    strUsage += HelpMessageOpt("-anonymizeCrowdFundingCoinamount=<n>", strprintf(_("Keep N CFUND anonymized (default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 48452, 39799));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2017-%i The CrowdFundingCoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
{
    RenameThread("CrowdFundingCoin-loadblk");
...
/** Sanity checks
 *  Ensure that CrowdFundingCoin is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    std::string strWalletFile = GetArg("-wallet", "cfund.dat");
#endif // ENABLE_WALLET
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. CrowdFundingCoin Core is shutting down."));
...
#endif
    // Make sure only a single CrowdFundingCoin process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. CrowdFundingCoin Core is probably already running."), strDataDir));
    ...
    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("CrowdFundingCoin version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (!fDisableWallet) {
            ...
            if (filesystem::exists(backupDir)) {
                ...
                for (boost::filesystem::directory_iterator dir_iter(backupFolder); dir_iter != end_iter; ++dir_iter) {
                    ...
                    if (boost::filesystem::is_regular_file(dir_iter->status())) {
                        ...
                        // Only add the backups for the current wallet, e.g. cfund.dat.*
        ...
        if (filesystem::exists(GetDataDir() / strWalletFile)) {
            ...
            if (r == CDBEnv::RECOVER_OK) {
                string msg = strprintf(_("Warning: cfund.dat corrupt, data salvaged!"
                                         " Original cfund.dat saved as wallet.{timestamp}.bak in %s; if"
            ...
            if (r == CDBEnv::RECOVER_FAIL)
                return InitError(_("cfund.dat corrupt, salvage failed"));
        }
...
// ********************************************************* Step 8: load wallet
    ...
    if (fDisableWallet) {
        ...
        if (GetBoolArg("-zapwallettxes", false)) {
            ...
            if (nZapWalletRet != DB_LOAD_OK) {
                uiInterface.InitMessage(_("Error loading cfund.dat: Wallet corrupted"));
                return false;
            }
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                strErrors << _("Error loading cfund.dat: Wallet corrupted") << "\n";
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR) {
                string msg(_("Warning: error reading cfund.dat! All keys read correctly, but transaction data"
                             " or address book entries might be missing or incorrect."));
                InitWarning(msg);
            } else if (nLoadWalletRet == DB_TOO_NEW)
                strErrors << _("Error loading cfund.dat: Wallet requires newer version of CrowdFundingCoin Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart CrowdFundingCoin Core to complete") << "\n";
                LogPrintf("%s", strErrors.str());
                return InitError(strErrors.str());
            } else
                strErrors << _("Error loading cfund.dat") << "\n";
        }
    ...
    nAnonymizeCrowdFundingCoinAmount = GetArg("-anonymizeCrowdFundingCoinamount", 0);
    if (nAnonymizeCrowdFundingCoinAmount > 999999) nAnonymizeCrowdFundingCoinAmount = 999999;
    if (nAnonymizeCrowdFundingCoinAmount < 2) nAnonymizeCrowdFundingCoinAmount = 2;
    ...
    LogPrintf("Anonymize CrowdFundingCoin Amount %d\n", nAnonymizeCrowdFundingCoinAmount);
