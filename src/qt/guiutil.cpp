...
#define URI_SCHEME "CrowdFundingCoin"
...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a CrowdFundingCoin address (e.g. %1)").arg("BL4YpGWPRWYW5LC8QhC3BbjToaTVTP8Waa"));
#endif
...
{
    // return if URI is not valid or is no CrowdFundingCoin: URI
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++) {
        ...
        if (i->first == "message") {
            ...
            if (!i->second.isEmpty()) {
                if (!BitcoinUnits::parse(BitcoinUnits::CFUND, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
{
    // Convert CrowdFundingCoin:// to CrowdFundingCoin:
    //
    //    Cannot handle this later, because CrowdFundingCoin:// will cause Qt to see the part after // as host,
...
QString formatBitcoinURI(const SendCoinsRecipient& info)
{
    ...
    if (info.amount) {
        ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::CFUND, info.amount, false, BitcoinUnits::separatorNever));
        paramCount++;
    }
...
void openConfigfile()
{
    ...
    /* Open CrowdFundingCoin.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "CrowdFundingCoin.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for CrowdFundingCoin.lnk
...
{
    return GetAutostartDir() / "CrowdFundingCoin.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a CrowdFundingCoin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=CrowdFundingCoin\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the CrowdFundingCoin app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add CrowdFundingCoin app to startup item list
